...
\section{Wireless mesh networks}% {{{2
...
Finally the author placed PS/2 keyboard and HD4780 LCD connectors for future enhancements. The complete schematic of the HopeMesh hardware is shown in figures \ref{fig:hopemesh-sch} and \ref{fig:hopemesh-levelshifter-sch}.
...
where
\begin{align*}
\#states &= \text{count of states} \\
\#locations &= \text{count of program locations} \\
\abs{dom(x)} &= \text{the domain of variable $x$}
\end{align*}
...
\pagebreak
\begin{samepage} % doesn't seem to work, need to insert pagebreak above
...
\section{Layer 3: B.A.T.M.A.N. Routing}% {{{2
\label{ch:batman}
...
\section{Performance measurements}% {{{2
\label{ch:performance_measurement}
...
The author concludes that the following achievements defined in the previous chapters have been achieved:

\begin{itemize}
    \item \textbf{Hardware design and implementation:} As described in chapter \ref{ch:hw} a new hardware design was researched and implemented by the author where pure electrical, CPU and RAM improvements were achieved.
    \item \textbf{Software architecture design:} As shown in chapter \ref{sec:module_architecture} a modular architecture was designed resulting in an improved maintainability.
    \item \textbf{Module concurrency algorithms:} One of the most critical aspects of the software implementation, the concurrent behaviour of software modules, was researched in chapter \ref{ch:concurrency}. An improved thread-based implementation was chosen.
    \item \textbf{Petri net research and simulations:} Chapter \ref{sec:petri} reveals a concurrent model for the radio module access using a petri net. A simulation for verification was presented.
    \item \textbf{Network stack design, implementation and simulation:} A complete network stack was designed and implemented as shown in chapter \ref{ch:network_stack}. Furthermore a simulation was implemented as shown in chapter \ref{ch:simulations}.
    \item \textbf{Route algorithm implementation and simulation:} A B.A.T.M.A.N. based routing algorithm was designed in chapter \ref{ch:batman} and simulated in chapter \ref{ch:routing_simulation}.
    \item \textbf{Performance measurements:} At last performance measurements were done in order to verify the theoretical research and assumptions as shown in chapter \ref{ch:performance_measurement}.
\end{itemize}

Nevertheless there are some open issues which could be resolved in future research or thesis works. The following aspects are left open:
